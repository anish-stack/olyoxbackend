...
// Get All Banners (uses cache)
exports.getAllAppHomeBanners = async (req, res) => {
  try {
    if (bannerCache.has(CACHE_KEY)) {
      return res.status(200).json({
        success: true,
        data: bannerCache.get(CACHE_KEY)
      });
    }

    const banners = await AppHomeBanner.find().sort({ createdAt: -1 }).lean();

    // Store in cache
    bannerCache.set(CACHE_KEY, banners);
...
exports.updateAppHomeBanner = async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      const uploadedImage = await uploadSingleImage(req.file.buffer);
      banner.image = { url: uploadedImage.image, public_id: uploadedImage.public_id };
    }
