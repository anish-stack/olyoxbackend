...
const {
  NewcreateRequest,
  BookingDetailsAdmin,
  ride_status_after_booking,
  riderFetchPoolingForNewRides,
  riderActionAcceptOrRejectRide,
  ride_status_after_booking_for_drivers,
  changeCurrentRiderRideStatus,
  verifyRideOtp,
  collectPayment,
  cancelRideByPoll,
  RateYourRider,
  cancelRideRequest,
  FetchAllBookedRides,
  AdminChangeCurrentRiderRideStatus,
  FindRiderNearByUser,
...
const {
  ...
  reCalculatePriceForOnlyRentals,
} = require("../../src/New-Rides-Controller/FindPrice");
const {
  updateRiderPreferences,
  getRiderPreferences,
} = require("../../src/New-Rides-Controller/V3/RIderPrefrencesUpdate");
const {
  bookIntercityRide,
  ...
  IntercityRideAll,
  getDriversForRide,
} = require("../../src/v3/IntercityRides");
const { getAllRides } = require("../../controllers/ride.request");
const { createPostRide,
  getMyPostRides,
  getPostRideById,
  updateRideStatus,
  cancelPostRide,
  getNearbyPostRides, } = require("../../controllers/postRideController");
...
NewRoutes.post(
  "/ride-action-reject-accepet-via/:rideId/:token/:action",
  riderActionAcceptOrRejectRideVia
);
...
NewRoutes.post("/post-ride", Protect, createPostRide);
NewRoutes.get("/post-ride/my-rides", Protect, getMyPostRides);
NewRoutes.get("/post-ride/nearby", Protect, getNearbyPostRides);
NewRoutes.get("/post-ride/:id", Protect, getPostRideById);
NewRoutes.patch("/post-ride/:id/status", Protect, updateRideStatus);
NewRoutes.patch("/post-ride/:id/cancel", Protect, cancelPostRide);
