...
const {
  ...
  getMyAllDetails,
  getMyAllRides,
  toggleWorkStatusOfRider,
  verifyDocument,
  uploadPaymentQr,
  getMySessionsByUserId,
  riderDocumentsVerify,
  updateBlockStatus,
  getSingleRider,
  updateRiderDetails,
  updateRiderDocumentVerify,
  logoutRider,
  deleteRider,
  AdmintoggleWorkStatusOfRider,
  saveFcmTokenToken,
  assignFreeRechargeToRider,
  addOnVehicle,
  getMyAddOnVehicle,
  updateDriverVehicleAddsOn,
  updateVehicleDetailsForDriver,
  getAddOnVehicleAdmin,
  approveVehicleDocument,
  getAllAddOnVehicleAdmin,
...
const {
  ...
  logout,
  addFcm,
} = require("../user_controller/user.register.controller");
const {
  make_recharge,
  verify_recharge,
} = require("../PaymentWithWebDb/razarpay");
const {
...
router.get("/driver-statics",getAllRidersStatics)
...
router.put(
  "/vehicles/:vehicleId/documents/:documentType/approve",
  approveVehicleDocument
);
...
router.post("/:rideId/messages", async (req, res) => {
  try {
    ...
    // Fetch ride with both user and driver FCM tokens
    const ride = await NewRideModelModel.findById(rideId)
