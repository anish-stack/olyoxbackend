...
exports.NewcreateRequest = async (req, res) => {
  try {
    ...
    console.log("Cashback Debug →", fare);
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        ...
        const preferenceFilteredRiders = riders.filter((rider) => {
          ...
          const normalizedDriverType = driverType?.trim() || "";
          const normalizedVehicleType = vehicleType?.trim() || "";

          let decision = false;

          switch (normalizedVehicleType.toUpperCase()) {
            case "MINI":
              decision =
                /MINI/i.test(normalizedDriverType) ||
                (/SEDAN/i.test(normalizedDriverType) &&
                  prefs.OlyoxAcceptMiniRides?.enabled) ||
                (/(SUV|XL|SUV\/XL)/i.test(normalizedDriverType) &&
                  prefs.OlyoxAcceptMiniRides?.enabled);
              break;
            ...
            case "SUV/XL":
            case "XL":
              decision = /(SUV|XL|SUV\/XL)/i.test(normalizedDriverType);
              break;
        ...
        preferenceFilteredRiders.forEach((rider) => {
          console.info(
            `✅ Rider Matched: ${rider.name || "Unnamed"} (${
              rider._id
            }) - Vehicle: ${rider?.rideVehicleInfo?.vehicleType}`
          );
        });
...
exports.cancelRideRequest = async (req, res) => {
  try {
    ...
    if (["completed", "cancelled"].includes(foundRide.ride_status)) {
      return res
        .status(400)
        .json({ message: "Ride is already completed or cancelled." });
    }
...
exports.riderActionAcceptOrRejectRideVia = async (req, res) => {
  try {
    ...
    console.log(
      `👉 Rider action request → Rider: ${riderId}, Ride: ${rideId}, Action: ${action}`
    );
    ...
    try {
      ...
      return res
        ...
        .json({
          success: false,
          message: "DB error fetching rider",
          error: err.message,
        });
    }
    ...
    if (!rider.isAvailable || rider.on_ride_id) {
      console.error("❌ Rider not available or already on ride:", {
        riderId,
        isAvailable: rider.isAvailable,
        onRide: rider.on_ride_id,
      });
      return res.status(400).json({
        success: false,
    ...
    try {
      ...
      return res
        ...
        .json({
          success: false,
          message: "DB error fetching ride",
          error: err.message,
        });
    }
...
cron.schedule("*/10 * * * * *", async () => {
  try {
    ...
    for (const ride of allRides) {
      console.log(
        `⛔ Cancelling ride ${ride._id} requested at ${ride.requested_at}`
      );
